...
Unix based operating systems are made up of two parts (Kernel and Shell), besides the core programs:
...
### Cursor
Indicates where in the line you are typing. Provided with some text, you can move around the cursor as desired.
...
* multiple *options* are allowed and you can squeeze them together:
{% highlight bash %}
ls -a  -l -h = ls -lah
{% endhighlight %}
* sometimes *options* require its own arguments so those will come right next to its option and may confuse the basic/standard structure, therefore it's common to see options arguments being passed with no spaces in between:
{% highlight bash %}
...
In the next post we will explore more of the Unix file system, understand its representation and manipulate several files. 